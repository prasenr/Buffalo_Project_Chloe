//
//  ContactFacebookAccountListViewController.m
...
@interface ContactFacebookAccountListViewController ()
...
@implementation ContactFacebookAccountListViewController
...
-(void) person:(PersonModel *)person {
    ...
    if([person.facebookAccounts count]>0) {
...
-(UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    FacebookAccountHistoryModel *aAccountHistory = [self.person.facebookAccounts objectAtIndex:indexPath.row];
    FacebookAccountModel *aAccountModel = aAccountHistory.account;
...
-(NSInteger) tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return [self.person.facebookAccounts count];
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    headerLabel.text = @"Facebook Accounts";
...
-(void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    FacebookAccountModel *contactFacebookAccount = [self.person.facebookAccounts objectAtIndex:indexPath.row];
    NSDictionary *userInfo = [NSDictionary dictionaryWithObject:contactFacebookAccount forKey:@"address"];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"showContactFacebookEditor" object:nil userInfo:userInfo];
...
-(void) createNoAccountsView {
    ...
    [addAccountButton setTitle:@"  add facebook account" forState:UIControlStateNormal];
    ...
    if([self.person.facebookAccounts count]>0) {
        [self.noAccountsView removeFromSuperview];
    } else {
        [self.view addSubview:self.noAccountsView];
    }
...
-(IBAction)addAccountClicked:(id)sender {
    FacebookAccountHistoryModel *contactFacebookAccount = [[FacebookAccountHistoryModel alloc] init];
    NSDictionary *userInfo = [NSDictionary dictionaryWithObject:contactFacebookAccount forKey:@"facebookAccount"];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"showContactFacebookEditor" object:nil userInfo:userInfo];
}

-(IBAction)onCancelClicked:(id)sender {
    [[NSNotificationCenter defaultCenter] postNotificationName:@"cancelContactFacebookAccountsList" object:nil];
}

-(void) addFacebookAccount:(FacebookAccountHistoryModel *)facebookAccount {
    
    [self.person.facebookAccounts addObject:facebookAccount];
