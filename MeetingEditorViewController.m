...
@interface MeetingEditorViewController ()
@property (nonatomic, retain) TodaySummaryListItems *meeting;
...
@implementation MeetingEditorViewController
...
- (void)viewDidLoad
{
    ...
    if(![self.meeting.locationLatLon[0] isEqualToString:@""]) {
        self.isFirstUpdate = YES;
        [self.locationManager startUpdatingLocation];
        
        
    } else {
        self.currentLocation = [[CLLocation alloc] initWithCoordinate: CLLocationCoordinate2DMake([self.meeting.locationLatLon[0] doubleValue], [self.meeting.locationLatLon[1] doubleValue])
                                                             altitude:0
    ...
    if(self.meeting.start_date != [[NSDate alloc] initWithTimeIntervalSinceNow:0]) {
        ...
        self.startTimeLabel.text = [NSString stringWithFormat:@"%@", [dateFormatter stringFromDate:self.meeting.start_date]];
    ...
    self.detailTextLabel.text = self.meeting.summaryItem;
...
-(IBAction)onOkClicked:(id)sender {
    NSDictionary *userInfo = [NSDictionary dictionaryWithObject:self.meeting forKey:@"meeting"];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"meetingCompleted" object:nil userInfo:userInfo];
}

-(IBAction)onDeleteClicked:(id)sender {
    NSDictionary *userInfo = [NSDictionary dictionaryWithObject:self.meeting forKey:@"meeting"];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"deleteMeeting" object:nil userInfo:userInfo];
}
...
-(IBAction)onCancelClicked:(id)sender {
    [[NSNotificationCenter defaultCenter] postNotificationName:@"cancelMeeting" object:nil];
}
