//
//  ContactInstantMessengerListViewController.m
...
@interface ContactInstantMessengerListViewController ()
...
@implementation ContactInstantMessengerListViewController
...
-(void) person:(PersonModel *)person {
    ...
    if([person.instantMessengerAccounts count]>0) {
...
-(UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    InstantMessengerAccountHistoryModel *aAccountHistory = [self.person.instantMessengerAccounts objectAtIndex:indexPath.row];
    InstantMessengerModel *aAccountModel = aAccountHistory.account;
...
-(NSInteger) tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return [self.person.instantMessengerAccounts count];
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    headerLabel.text = @"Instant Messenger Accounts";
...
-(void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    InstantMessengerModel *contactInstantMessengerAccount = [self.person.instantMessengerAccounts objectAtIndex:indexPath.row];
    NSDictionary *userInfo = [NSDictionary dictionaryWithObject:contactInstantMessengerAccount forKey:@"address"];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"showContactInstantMessengerEditor" object:nil userInfo:userInfo];
...
-(void) createNoAccountsView {
    ...
    [addAccountButton setTitle:@"  add instant messenger account" forState:UIControlStateNormal];
    ...
    if([self.person.instantMessengerAccounts count]>0) {
        [self.noAccountsView removeFromSuperview];
    } else {
        [self.view addSubview:self.noAccountsView];
    }
...
-(IBAction)addAccountClicked:(id)sender {
    InstantMessengerAccountHistoryModel *contactInstantMessengerAccount = [[InstantMessengerAccountHistoryModel alloc] init];
    NSDictionary *userInfo = [NSDictionary dictionaryWithObject:contactInstantMessengerAccount forKey:@"instantMessengerAccount"];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"showContactInstantMessengerEditor" object:nil userInfo:userInfo];
}

-(IBAction)onCancelClicked:(id)sender {
    [[NSNotificationCenter defaultCenter] postNotificationName:@"cancelContactInstantMessengerAccountsList" object:nil];
}

-(void) addInstantMessengerAccount:(InstantMessengerAccountHistoryModel *)instantMessengerAccount {
    [self.person.instantMessengerAccounts addObject:instantMessengerAccount];
