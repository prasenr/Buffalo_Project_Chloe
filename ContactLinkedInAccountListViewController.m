//
//  ContactLinkedInAccountListViewController.m
...
@interface ContactLinkedInAccountListViewController ()
...
@implementation ContactLinkedInAccountListViewController
...
-(void) person:(PersonModel *)person {
    ...
    if([person.linkedInAccounts count]>0) {
...
-(UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    LinkedInAccountHistoryModel *aAccountHistory = [self.person.linkedInAccounts objectAtIndex:indexPath.row];
    LinkedInAccountModel *aAccountModel = aAccountHistory.account;
...
-(NSInteger) tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return [self.person.linkedInAccounts count];
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    headerLabel.text = @"LinkedIn Accounts";
...
-(void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    LinkedInAccountModel *contactLinkedInAccount = [self.person.linkedInAccounts objectAtIndex:indexPath.row];
    NSDictionary *userInfo = [NSDictionary dictionaryWithObject:contactLinkedInAccount forKey:@"address"];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"showContactLinkedInEditor" object:nil userInfo:userInfo];
...
-(void) createNoAccountsView {
    ...
    [addAccountButton setTitle:@"  add linked in account" forState:UIControlStateNormal];
    ...
    if([self.person.linkedInAccounts count]>0) {
        [self.noAccountsView removeFromSuperview];
    } else {
        [self.view addSubview:self.noAccountsView];
    }
...
-(IBAction)addAccountClicked:(id)sender {
    LinkedInAccountHistoryModel *contactLinkedInAccount = [[LinkedInAccountHistoryModel alloc] init];
    NSDictionary *userInfo = [NSDictionary dictionaryWithObject:contactLinkedInAccount forKey:@"linkedInAccount"];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"showContactLinkedInEditor" object:nil userInfo:userInfo];
}

-(IBAction)onCancelClicked:(id)sender {
    [[NSNotificationCenter defaultCenter] postNotificationName:@"cancelContactLinkedInAccountsList" object:nil];
}

-(void) addLinkedInAccount:(LinkedInAccountHistoryModel *)linkedInAccount {
    [self.person.linkedInAccounts addObject:linkedInAccount];
