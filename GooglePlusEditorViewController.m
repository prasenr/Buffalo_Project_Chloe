//
//  GooglePlusEditorViewController.m
...
@interface GooglePlusEditorViewController ()
@property (nonatomic, strong) GooglePlusAccountHistoryModel *account;
@property (nonatomic, strong) UITextField *accountInput;
@end

@implementation GooglePlusEditorViewController
...
- (void)viewDidLoad {
    ...
    headerLabel.text = @"Add Google+ Account";
    ...
    self.accountInput.placeholder = @"google+ account";
...
-(BOOL)textFieldShouldReturn:(UITextField *)textField {
    ...
    if([self.accountInput.text length] > 0) {
        GooglePlusAccountHistoryModel *googlePlusHistory = [[GooglePlusAccountHistoryModel alloc] init];
        GooglePlusAccountModel *googlePlusAccount = [[GooglePlusAccountModel alloc] init];
        googlePlusAccount.username = [NSMutableString stringWithString:self.accountInput.text];
        googlePlusHistory.account = googlePlusAccount;
        
        NSDictionary *userInfo = [NSDictionary dictionaryWithObject:googlePlusHistory forKey:@"googlePlusAccount"];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"addGooglePlusAccount" object:nil userInfo:userInfo];
    }
    return YES;
}

-(IBAction)onCancelClicked:(id)sender {
    [[NSNotificationCenter defaultCenter] postNotificationName:@"cancelGooglePlusEditor" object:nil];
}
