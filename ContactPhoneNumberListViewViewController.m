//
//  ContactPhoneNumberListViewViewController.m
...
@interface ContactPhoneNumberListViewViewController ()
...
@implementation ContactPhoneNumberListViewViewController
...
-(UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    PhoneNumberHistoryModel *aAccountHistory = [self.person.phoneNumbers objectAtIndex:indexPath.row];
    PhoneNumberModel *aAccountModel = aAccountHistory.account;
    ...
    NSMutableString *phoneText = [NSMutableString stringWithFormat:@""];
    if([aAccountModel.country length]>0) {
        [phoneText appendString:@"+"];
        [phoneText appendString:aAccountModel.country];
        [phoneText appendString:@" "];
    }
    
    if([aAccountModel.areaCode length]>0) {
        [phoneText appendString:@"("];
        [phoneText appendString:aAccountModel.areaCode];
        [phoneText appendString:@") "];
    }
    
    if([aAccountModel.prefix length]>0) {
        [phoneText appendString:aAccountModel.prefix];
        [phoneText appendString:@"-"];
    }
    
    if([aAccountModel.phoneNumber length]>0) {
        [phoneText appendString:aAccountModel.phoneNumber];
    }
    
    if([aAccountModel.extension length]>0) {
        [phoneText appendString:@"ext. "];
        [phoneText appendString:aAccountModel.extension];
    }
    
    self.accountLabel.text = phoneText;
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    headerLabel.text = @"Phone Numbers";
...
-(void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    PhoneNumberModel *contactPhoneNumber = [self.person.phoneNumbers objectAtIndex:indexPath.row];
    NSDictionary *userInfo = [NSDictionary dictionaryWithObject:contactPhoneNumber forKey:@"address"];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"showContactPhoneNumberEditor" object:nil userInfo:userInfo];
...
-(void) createNoAccountsView {
    ...
    [addAccountButton setTitle:@"  add phone number" forState:UIControlStateNormal];
...
-(IBAction)addAccountClicked:(id)sender {
    PhoneNumberHistoryModel *contactPhoneNumber = [[PhoneNumberHistoryModel alloc] init];
    NSDictionary *userInfo = [NSDictionary dictionaryWithObject:contactPhoneNumber forKey:@"phoneNumber"];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"showContactPhoneNumberEditor" object:nil userInfo:userInfo];
}

-(IBAction)onCancelClicked:(id)sender {
    [[NSNotificationCenter defaultCenter] postNotificationName:@"cancelContactPhoneNumbersList" object:nil];
}

-(void) addPhoneNumber:(PhoneNumberHistoryModel *)phoneNumber {
    [self.person.phoneNumbers addObject:phoneNumber];
