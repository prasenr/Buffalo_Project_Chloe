//
//  ContactTwitterAccountListViewController.m
...
@interface ContactTwitterAccountListViewController ()
...
@implementation ContactTwitterAccountListViewController
...
-(void) person:(PersonModel *)person {
    ...
    if([person.twitterAccounts count]>0) {
...
-(UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    TwitterAccountHistoryModel *aAccountHistory = [self.person.twitterAccounts objectAtIndex:indexPath.row];
    TwitterAccountModel *aAccountModel = aAccountHistory.account;
    ...
    if (indexPath.row % 2) {
        cell.contentView.backgroundColor = [[UIColor alloc]initWithRed:36.0/255.0 green:36.0/255.0 blue:36.0/255.0 alpha:1];
    } else {
        cell.contentView.backgroundColor = [[UIColor alloc]initWithRed:56.0/255.0 green:56.0/255.0 blue:56.0/255.0 alpha:1];
    }
...
-(NSInteger) tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return [self.person.twitterAccounts count];
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    headerLabel.text = @"Twitter Accounts";
...
-(void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    TwitterAccountHistoryModel *contactTwitterAccount = [self.person.twitterAccounts objectAtIndex:indexPath.row];
    NSDictionary *userInfo = [NSDictionary dictionaryWithObject:contactTwitterAccount forKey:@"address"];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"showContactTwitterEditor" object:nil userInfo:userInfo];
...
-(void) createNoAccountsView {
    ...
    [addAccountButton setTitle:@"  add twitter accounts" forState:UIControlStateNormal];
    ...
    if([self.person.twitterAccounts count]>0) {
        [self.noAccountsView removeFromSuperview];
    } else {
        [self.view addSubview:self.noAccountsView];
    }
...
-(IBAction)addAccountClicked:(id)sender {
    TwitterAccountHistoryModel *contactTwitterAccount = [[TwitterAccountHistoryModel alloc] init];
    NSDictionary *userInfo = [NSDictionary dictionaryWithObject:contactTwitterAccount forKey:@"address"];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"showContactTwitterEditor" object:nil userInfo:userInfo];
}
-(IBAction)onCancelClicked:(id)sender {
    [[NSNotificationCenter defaultCenter] postNotificationName:@"cancelTwitterAccountsList" object:nil];
}

-(void) addTwitterAccount:(TwitterAccountHistoryModel *)twitterAccount {
    [self.person.twitterAccounts addObject:twitterAccount];
